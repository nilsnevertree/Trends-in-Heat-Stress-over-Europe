...
#%% ==================== CALCULATION OF SWBGT for slices================
#calculate swbgt
...
temporary = data.sel({"latitude" : slice(lat[idx+1], lat[idx])}) #, longitude= slice(lon[idx], lon[idx+1]) )
st = time.time()
swbgt = swbgt_func(temporary.t2m, temporary.d2m)
swbgt.name = "swbgt"
xr.merge([swbgt])
...
# swbt_on_disk = xr.open_dataset(r"ccc.nc")

# print("stop?")
for idx in [1,2]:
        print(slice(lat[idx], lat[idx+1]))
        print(slice(lon[idx], lon[idx+1]))
        temporary = data.sel(latitude= slice(lat[idx+1], lat[idx])) #, longitude= slice(lon[idx], lon[idx+1]) )
        st = time.time()
        swbgt = swbgt_func(temporary.t2m, temporary.d2m)
        swbgt.name = "swbgt"
...
netcdf_data = xr.open_dataset(save_dict["savefolder"] + save_dict["savename"])
...
for quant in quantiles :
        ...
        swbgt_quant_mean = swbgt.where(swbgt >= swbgt.quantile(quant, dim = "time"), drop=True).chunk(rechunk_dim).mean(dim= "time")
        print("=======\ncalc sWGBT {} mean :\n{:.1f}s".format(percentile, processing_time))
