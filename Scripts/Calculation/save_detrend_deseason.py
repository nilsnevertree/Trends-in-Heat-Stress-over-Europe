...
def save_calc_swbgt(save_dict, load_dict, save = True) :
        ...
        if longitude_slice.start < 0 :
                ...
                print("calculated west side")
                print("=============\nDATA WEST",data_west.dims)
                input("WEST OK?")
                data_all = xr.merge([data_east, data_west])
                print("=============\nDATA ALL",data_all.dims)
                if save: 
...
def load_t2m_d2m_calc_swbgt(save_dict, load_dict) :
        ...
        chunks = save_dict["rechunked"]
        print("=======\ncalculate and save data")
        # -------- load data from path --------
        try :
                st = time.time()
                t2m = xr.open_dataset(load_dict["t2m_path"]) #.chunks={"time" : 40000}
                print("load temp took {:.2f}s".format(time.time()-st))
                st = time.time()
                d2m = xr.open_dataset(load_dict["d2m_path"]) #.chunks={"time" : 40000}
        ...
        # -------- rechunk data in as many chunks as wanted --------
        if save_dict["rechunked"]:
                print("-------\chunks used along time:" +str(chunks) +"\n")
                rechunk_dim = {
                "time" : int(np.shape(org_data.t2m)[0] / chunks),
                }
                # ======== USE  °C  ========
...
#         hi_index = save_dict.setdefault("tws", False)
        quantiles = save_dict.setdefault("quantiles", False)
        deseason = save_dict.setdefault("deseason" , False)
        detrend = save_dict.setdefault("detrend" , False)
        added = False
        ...
        if detrend is not False:
                ...
                for variable in variable_list :
                        try :
                                data[varianle][0]
                                print(variable + "_detrend found")
                        except:
                                trends.gufunc_detrend(data= data, variable= variable)
                                print(variable + "_detrend found")
                                added = True
        else :
                print('no detrend')
        ...
        if quantiles is not False:
                ...
                for variable in variable_list :
                        try :
                                ...
                                print(variable  + "_mask_quantiles found")
                        except:
                                data[variable  + "_mask_quantiles"] = data[variable] >= data[variable].quantile(quantile_list, dim = "time")
                                print(variable  + "_mask_quantiles added")
                                added = True
        else :
                print('no quantile')
